...
for filename in os.listdir(video_directory):
    ...
    # columns: 0…grid_cols-1, rows: 0…grid_rows-1
    # we treat each cell as a square of size 1 in this coordinate system
    x = np.arange(0.5, grid_cols, 1.0)   # e.g. [0.5,1.5,...,15.5]
    y = np.arange(0.5, grid_rows, 1.0)   # e.g. [0.5,1.5,...,8.5]
    X, Y = np.meshgrid(x, y)

    # contour levels (you can adjust number or exact values)
    levels = 14

    # filled contour with single colormap
    cf = ax.contourf(X, Y, movement_grid, levels=levels, cmap="Reds")

    # contour lines in darker blue
    cs = ax.contour(X, Y, movement_grid, levels=levels, colors='#63111bff', linewidths=0.5)
    ...
    for i in range(grid_rows):
        for j in range(grid_cols):
            ax.text(j+0.5, i+0.5, f"{movement_grid[i,j]:.0f}",
    ...
    ax.set_xticklabels(col_labels)
    ax.set_yticklabels(row_labels)
    ax.set_title(f"Shrimp Movement Contour: {base_name}")
    ax.set_xlabel("Grid Column")
    ax.set_ylabel("Grid Row")
    ax.invert_yaxis()  # optional: origin at top
    ax.set_aspect(9/16) #Ensure that the x-axis is not compressed
    plt.tight_layout()
