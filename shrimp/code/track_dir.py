...
# The actual size of the shrimp (unit: mm)
actual_shrimp_width_mm = 10
capture_number = 0
shrimp_number = 0
smoothed_window_size = 10
model=YOLO('../runs/detect/train26/weights/best.pt')  # Change the model path

video_directory="E:/hydrothermal/2017-2018/2017_2018_mp4/SMOOVE-17-07-23_00-00-23-80.mp4"
...
for filename in os.listdir(video_directory):
    if filename.endswith(('.mp4','.mkv')):
        ...
        # Read the width, height, fps of the video
        ...
        # Proportional Dictionary: Save the pixel-to-millimeter conversion coefficient for each shrimp
        pixel_to_mm_ratios = {}
            ...
            if results[0].boxes.id is not None:
                ...
                for box,track_id,cls,conf in zip(boxs,track_ids,clss,confs):
                    if cls == 1 and conf>=0.5:  # 0:mussel, 1:shrimp
                        ...
                        x1,y1,x2,y2=box
                        bbox_width_px = x2 - x1
                        # Calculate the conversion coefficient from pixels to millimeters (calculated separately for each target)
                        ...
                        if len(raw_tracks[track_id]) >= smoothed_window_size:
                            smoothed=np.mean(raw_tracks[track_id][-smoothed_window_size:],axis=0)  # Take the average of the last N values
                        else:
                            smoothed = (center_x,center_y)
                        smoothed_tracks[track_id].append(smoothed)
                        # Limit trajectory length
                        '''
                        If it exceeds the specified maximum value, remove the first point
                        '''
                        ...
                        if len(smoothed_tracks[track_id]) >= 2:
                            ...
                            if  distance_mm>1 and distance_mm <= 5 :  # Filter displacement less than 1mm and greater than 5mm
                                total_distances[track_id] += distance_mm
                                all_shrimp_distance += distance_mm
...
# Calculate average movement per shrimp per minute
if shrimp_number:
    avg_shrimp_permin_distance=(all_shrimp_distance/shrimp_number)
else:
    avg_shrimp_permin_distance = 0
# Get the time corresponding to the first and last frames
...
try:
    ...
    print("Failed to write result2.txt:", e)
