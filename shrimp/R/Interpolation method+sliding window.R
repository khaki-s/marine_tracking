...
# 2. Convert time column to Date
df_daily <- df_raw %>%
  mutate(time = as.Date(time)) %>%
  ungroup()

# 3. Generate a complete date sequence from earliest to latest
...
# 4. Left join, fill missing dates with NA
df_full <- all_dates %>%
  left_join(df_daily, by = "time") %>%
  arrange(time)

# 5. Linear interpolation to fill NA values
...
# 6.sliding window average, right-aligned
...
# 7. For rows at the beginning that are less than the window size, directly fill with interpolated values
