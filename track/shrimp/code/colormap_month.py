...
out_directory = "/home/jingyichu/app/marine-tracking/track/shirmp/colormap/2018-2019"
#correlation_path = "/home/jingyichu/app/ultralytics-8.3.67/shrimp/colormap_correlation/correlation/2020-2021.txt"
...
vent_type = 'point'# point or line

# Hydrothermal vent location (can be point or line)
# For point vent:
vent_point = (12.5, 3.5)  # (x, y) coordinates in grid units
...
for month_key, file_list in video_files_by_month.items():
    ...
    for filename in file_list:
        ...
        # Dictionaries to collect shrimp trajectories per track
        shrimp_tracks = defaultdict(list)
        # -------------------------------------------------------
        # [CHANGED] body_lengths_px: stores the EMA of bounding-box
        # diagonal (pixels) for each track, used to convert pixel
        # distances to Body-Length (BL) units — same approach as
        # track.py.  Replaces the old pixel_to_mm_ratios dict.
        # -------------------------------------------------------
        body_lengths_px = {}

        # Process each frame to collect shrimp center positions
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
                break
            ...
            for box, cls, track_id, conf in zip(boxes, clss, ids, confs):
                if cls == 1 and conf >= 0.5:  # process only shrimp detections
                    ...
                    # [CHANGED] Use bounding-box diagonal as body length (px),
                    # updated with an exponential moving average (EMA) identical
                    # to track.py:
                    #   first detection  → initialise directly
                    #   subsequent frames → 0.8 * old + 0.2 * new
                    body_length_px = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
        ...
        # Initialize a grid for movement accumulation; each cell holds total
        # movement distance expressed in Body Lengths (BL).
        ...
        for track_id, points in shrimp_tracks.items():
            ...
            # For each segment, compute distance (in BL) and the midpoint
            for i in range(1, len(smoothed)):
                pt_prev = smoothed[i - 1]
                pt_curr = smoothed[i]
                seg_dist_px = np.linalg.norm(pt_curr - pt_prev)

                # [CHANGED] Convert pixel distance → BL using the per-track
                # EMA body length, mirroring track.py's logic exactly.
                bl = body_lengths_px.get(track_id, 0)
                if bl > 0:
                    seg_dist_bl = seg_dist_px / bl
                else:
                    seg_dist_bl = 0

                # [CHANGED] Apply the same validity filter as track.py:
                # discard micro-jitter (≤0.02 BL) and large jumps (>0.5 BL).
                if not (0.02 < seg_dist_bl <= 0.5):
                    continue
                ...
                if 0 <= col_idx < grid_cols and 0 <= row_idx < grid_rows:
                    # [CHANGED] Accumulate BL instead of mm
                    movement_grid[row_idx, col_idx] += seg_dist_bl
...
# with open(correlation_path,"w+",encoding="utf-8") as f1:
...
plt.ylabel('Movement (BL)')  # [CHANGED] mm → BL
...
for month_key, avg_movement_grid in monthly_grids.items():
    ...
    # contour levels 
    levels = np.linspace(0, global_max, 15)  # unified colorbar across all months
    ...
    cbar = fig.colorbar(cf, ax=ax, orientation="vertical",
                        label="Movement (BL)")  # [CHANGED] mm → BL
    ...
    for i in range(grid_rows):
        for j in range(grid_cols):
            ax.text(j+0.5, i+0.5, f"{avg_movement_grid[i,j]:.2f}",  # [CHANGED] .0f→.2f for BL precision
